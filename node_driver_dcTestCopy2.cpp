...
bool terminableR, terminableS; // bool variable to signal receiver & sender's termination
...
terminableR = false; // initiate var terminable
terminableS = false;
...
while ( (numOfCompletedNodes < num_nodes) || (hasIncomingQueue) )
{
    ...
    // only do the processing if incoming queue has something
    {
        ...
        //cout << "discover count:\t" << numOfDiscoveredNodes << endl;
        //cout << "complete count:\t" << numOfCompletedNodes << endl;
    }
...
void sender(Node *this_node)
{
   // TODO - sender needs to setup socket connections with all of its neighbors
    ...
    for (int i = 0; i < countOf1Hop; i++)
    {
        ...
        while (connect( client_sd[i], (struct sockaddr *) &client_addr[i], sizeof(client_addr[i]) ) < 0 )
        {        
    ...
    while ( (!terminableS) || (hasOutgoingQueue) )
    // while terminable is not set, or outgoing queue is not empty, do sending out msg from the queue
    {
        ...
        if(hasOutgoingQueue)
        {
            ...
            for (int i= 0; i< countOf1Hop; i++)
            {
                ...
                {
                    while ((outgoingSize = write(client_sd[i], &outgoingMsg, sizeof(outgoingMsg) )) <= 0)
                    {
...
void receiver(Node *this_node)
{
    ...
    int cl_sd[countOf1Hop]; // client socket desc
    ...
    {  
        ...
        {cout << "rec continues 4" << endl;
            ...
            for (int i = 0; i< countOf1Hop; i++)
            {
                // if client socket #i == 0, meaning it is empty
                if (cl_sd[i] == 0)
                {
                    cl_sd[i] = new_sd;
    ...
    terminableS = true; // signal terminable for sender thread    
    terminableR = false; // reset receiver terminable to false to wait for sender

    while(!(terminableR && terminableS))
    {
        // while two terminable vars are not both true, wait
    }
