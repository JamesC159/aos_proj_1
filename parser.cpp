	...
	while (std::getline(in, line))
	{
		if (Is_Valid_Line(line))
		{
			...
			else if (line_num < num_nodes + 1)
			{
				...
				Node *n = new Node(Node(std::stoi(tokens[0]), tokens[1], std::stoi(tokens[2])));
				node_map[std::stoi(tokens[0])] = n; 
			}
			else //Second n  
			{
				std::istringstream iss(line);
				...
				for (std::vector<std::string>::iterator it = tokens.begin() + 1; it != tokens.end(); it++)
				{
					...
					node_map[std::stoi(tokens[0])] -> insertAdjNode(node_map[std::stoi(*it)]);
				}
			}
